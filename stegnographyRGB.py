...
def main():
	...
	carrier_images=[]
	cover_images=[]
	root_dir = './Images/carrier images'
	...
	carrier_image_name="./Images/carrier images/3.jpg"
	...
	carrier_image_matrix=cv2.imread(carrier_image_name)  # Converting the RGB carrier image to grey scale
	print(carrier_image_matrix)
	...
	final_image_matrix = np.zeros((len(carrier_image_matrix),len(carrier_image_matrix[0]),len(carrier_image_matrix[0][0])),dtype=int)
	print(final_image_matrix)
	cover_image_name="./Images/cover images/1.png"
	...
	cv2.imwrite('Images/greyScale/greyscaleCoverImage' +'.jpg',cover_grey_image_matrix)
	...
	if len(carrier_image_matrix) <=2* len(cover_grey_image_matrix) or len(carrier_image_matrix[0]) <=2* len(cover_grey_image_matrix[0]):
	...
	while row_cover<len(cover_grey_image_matrix) and i < len(carrier_image_matrix): #Traversing the cover image matrix
		# Extraction of 2x2 non-overlapping matrix
		temp = [[carrier_image_matrix[i][j][0],carrier_image_matrix[i][j+1][0]],[carrier_image_matrix[i+1][j][0],carrier_image_matrix[i+1][j+1][0]]]
		...
		final_image_matrix[i][j][0] = temp2[0][0]  # The modified pixel values are stored in the final image matrix
		final_image_matrix[i][j+1][0] = temp2[0][1] # Values are in grey scale
		final_image_matrix[i+1][j][0] = temp2[1][0]
		final_image_matrix[i+1][j+1][0] = temp2[1][1]
		j+=2
		if j==len(carrier_image_matrix[0]):
			#print("j=",j)
			i+=2
			j=0
		if row_cover>=len(cover_grey_image_matrix):
			break
		temp = [[carrier_image_matrix[i][j][1],carrier_image_matrix[i][j+1][1]],[carrier_image_matrix[i+1][j][1],carrier_image_matrix[i+1][j+1][1]]]
		cover = cover_grey_image_matrix[row_cover][col_cover]
		col_cover+=1
		if col_cover==len(cover_grey_image_matrix[0]):
			# print("col",col_cover)
		...
		final_image_matrix[i][j][1] = temp2[0][0]  # The modified pixel values are stored in the final image matrix
		final_image_matrix[i][j+1][1] = temp2[0][1] # Values are in grey scale
		final_image_matrix[i+1][j][1] = temp2[1][0]
		...
		if row_cover>len(cover_grey_image_matrix):
			break
		temp = [[carrier_image_matrix[i][j][2],carrier_image_matrix[i][j+1][2]],[carrier_image_matrix[i+1][j][2],carrier_image_matrix[i+1][j+1][2]]]
		cover = cover_grey_image_matrix[row_cover][col_cover]
		col_cover+=1
		if col_cover==len(cover_grey_image_matrix[0]):
			# print("col",col_cover)
		...
		final_image_matrix[i][j][2] = temp2[0][0]  # The modified pixel values are stored in the final image matrix
		final_image_matrix[i][j+1][2] = temp2[0][1] # Values are in grey scale
		final_image_matrix[i+1][j][2] = temp2[1][0]
		final_image_matrix[i+1][j+1][2] = temp2[1][1]
		j+=2
		if j==len(carrier_image_matrix[0]):
			#print("j=",j)
			i+=2
	...
	for i in range(k,len(carrier_image_matrix)):
		for j in range(len(carrier_image_matrix[0])):
			for k in range(len(carrier_image_matrix[0][0])):
				final_image_matrix[i][j][k]=carrier_image_matrix[i][j][k]
	i-=2
	j-=2
	...
	# temp = [[carrier_image_matrix[i][j],carrier_image_matrix[i][j+1]],[carrier_image_matrix[i+1][j],carrier_image_matrix[i+1][j+1]]]
	...
	print('Storing the result in Result_' + '.png\n')
	img = Image.fromarray(np.uint8(final_image_matrix),'RGB') # Storing the final image matrix as a grey scale image
	img.save('results/Result_' + '.png')
	end_time=time.time()
